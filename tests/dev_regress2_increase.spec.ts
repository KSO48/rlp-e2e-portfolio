...
// ============================================================
// ПОЛНОЕ УВЕЛИЧЕНИЕ ЦЕНЫ ДЛЯ СТЕНДА DEV01 (<dev-stand-url>)
// Аналог test_regress2_increase.spec.ts — переиспользует обобщённый
// runFullApprovalChain (approvalChain.ts) с пользователями/baseUrl/
// проверкой маршрута ЭДО dev01.
//
// НЕ прогонялся целиком на dev01 — построен по образцу подтверждённого
// dev_smoke1_registration.spec.ts / dev_regress1_registration.spec.ts.
// ============================================================
...
test('test', async () => {
  ...
  try {
    ...
    await page.waitForTimeout(1000);
    await page.getByText('Перерегистрация ПОЦ в целях увеличения цены').click();
    await page.waitForTimeout(1000);
    ...
    await runFullApprovalChain(page, {
      ...
      finalDecisionText: 'Решение о государственной перерегистрации предельной отпускной цены в целях увеличения',
    ...
    const fileContent =
      '========================================\n' +
      'ОТЧЕТ О СОЗДАНИИ ЗАЯВЛЕНИЯ (Увеличение цены, регресс, dev01)\n' +
    ...
    console.log('\n✅ Регресс-тест увеличения цены на dev01 пройден!');
  } finally {
    await context.close();
  }
});
